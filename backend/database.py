...
DATABASE_URL = "sqlite+aiosqlite:///./ejentic_audit.db"
...
async def get_token_metrics(limit: int = 20):
    """Aggregate token accounting + the most recent per-query rows. Powers the
    GET /metrics endpoint so token spend is observable at any time."""
    async with AsyncSessionLocal() as session:
        total_queries = await session.scalar(select(func.count(AuditLog.id))) or 0
        gated = await session.scalar(
            select(func.count(AuditLog.id)).where(AuditLog.gated == 1)
        ) or 0
        answered = await session.scalar(
            select(func.count(AuditLog.id)).where(AuditLog.gated == 0)
        ) or 0
        sum_prompt = await session.scalar(
            select(func.coalesce(func.sum(AuditLog.prompt_tokens), 0))
        ) or 0
        sum_completion = await session.scalar(
            select(func.coalesce(func.sum(AuditLog.completion_tokens), 0))
        ) or 0
        sum_total = await session.scalar(
            select(func.coalesce(func.sum(AuditLog.total_tokens), 0))
        ) or 0
        sum_saved = await session.scalar(
            select(func.coalesce(func.sum(AuditLog.estimated_saved_tokens), 0))
        ) or 0
